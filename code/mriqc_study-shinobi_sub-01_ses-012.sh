...
#SBATCH --job-name=mriqc_study-shinobi_sub-01_ses-012.job
#SBATCH --output=./code/mriqc_study-shinobi_sub-01_ses-012.out
#SBATCH --error=./code/mriqc_study-shinobi_sub-01_ses-012.err
#SBATCH --time=2:00:00
...
datalad containers-run -m 'mriqc_sub-01/ses-012' -n bids-mriqc --input sourcedata/shinobi/sub-01/ses-012/fmap/ --input sourcedata/shinobi/sub-01/ses-012/func/ --output . -- -w workdir/ --participant-label 01 --session-id 012 --omp-nthreads 8 --nprocs 8 -m bold --mem_gb 32 --no-datalad-get  --no-sub sourcedata/shinobi ./ participant 
mriqc_exitcode=$?
